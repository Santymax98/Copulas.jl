...
Hypothesis that data follow a specified copula or fitted copula model. The field `kind` distinguishes `:simple`, `:composite`, and `:selected`.
"""
...
GoodnessOfFitHypothesis(M::CopulaModel) = GoodnessOfFitHypothesis(M, get(M.method_details, :selection, false) ? :selected : :composite)
...
function nullhypothesis(h::GoodnessOfFitHypothesis)
    ...
    h.kind === :selected && return "At least one candidate copula family adequately describes the dependence."
...
function _gof_refit(M::CopulaModel, U::AbstractMatrix)
    ...
    if get(md, :selection, false)
        return Distributions.fit(CopulaModel, Copula, U; candidates=md.candidates, criterion=md.criterion, method=md.requested_method,
                                 quick_fit=false, derived_measures=false, vcov=false, md.selection_options...)
    end
